...
# Let's fix the string interpolations the powershell script mangled first
content = content.replace('${l10n.pet_appointment_tab_partner}: $professional', '\: \')
content = content.replace('${l10n.pet_field_what_to_do}: $whatToDo', '\: \')
content = content.replace('\\\n\\n[\]: \', '\\\n\\n[\]: \')

# There's a severe bracket mismatch in the build method.
# Let's locate the build method's end and reconstruct the closures manually.
parts = content.split('                  itemBuilder: (context, index) {')
if len(parts) > 1:
    before = parts[0]
    # We will reconstruct everything from itemBuilder to _confirmDelete
    after_parts = parts[1].split('  void _confirmDelete(BuildContext context, String id) {')
    if len(after_parts) > 1:
        after = after_parts[1]
        
        # Reconstruct the widget tree cleanly
        clean_block = f'''                  itemBuilder: (context, index) {{
                                ...
                                children: [
                                  ...
                                  if (whatToDo.isNotEmpty) ...[
                                     ...
                                     Container(
                                       ...
                                       child: Text(
                                         f'\u0024{{l10n.pet_field_what_to_do}}: \u0024whatToDo',
                                  ...
                                  if (professional.isNotEmpty) ...[
                                     ...
                                     Row(
                                       children: [
                                         ...
                                         Expanded(
                                           child: Text(
                                             f'\u0024{{l10n.pet_appointment_tab_partner}}: \u0024professional', 
  ...
  void _confirmDelete(BuildContext context, String id) {{'''
        ...
        content = before + clean_block + after

# Need to apply the opacity to withValues migration globally matching Stability rule
