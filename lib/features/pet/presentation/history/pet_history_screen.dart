...
class _PetHistoryScreenState extends State<PetHistoryScreen> {
  ...
  void initState() {
    ...
    if (widget.petUuid != null) {
      if (kDebugMode)
        debugPrint(
            '${PetConstants.logTagPetData} [HISTORY] Inicializando tela em Modo Filtrado para: ${widget.petUuid}');
    } else {
      if (kDebugMode)
        debugPrint(
            '${PetConstants.logTagPetData} [HISTORY] Inicializando tela para TODOS (Modo Global)');
    }
  }

  void _updateFriendCache() {
    final friends = _petBox
        .query(PetEntity_.type.equals(PetConstants.typeFriend))
        .build()
        .find();
  ...
  Widget build(BuildContext context) {
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
              ...
              return AnimatedBuilder(
                  ...
                  builder: (context, child) {
                    return TabBar(
                      ...
                      indicatorColor: tabController.index == 1
                          ? const Color(0xFFE0BBE4)
                          : const Color(
                              0xFFFFD1DC), // Lilac for Friends, Pink for Pets
        ...
        body: StreamBuilder<List<PetHistoryEntry>>(
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              return const Center(
                  child: CircularProgressIndicator(color: Color(0xFF10AC84)));
            }

            if (snapshot.hasError) {
              return Center(
                  child: Text(
                      l10n.pet_error_history_load(snapshot.error.toString()),
            ...
            if (widget.petUuid == null && myPetsEntries.isNotEmpty) {
              ...
              for (var entry in myPetsEntries) {
                final key = (entry.petUuid.isEmpty ||
                        entry.petUuid == PetConstants.tagEnvironment)
                    ? entry.petName
                    : entry.petUuid;
        ...
        floatingActionButton: widget.petUuid != null
                            ...
                            onTap: () {
                              Navigator.pushNamed(
                                ...
                                arguments: {
                                  ...
                                  PetConstants.argImagePath:
                                      widget.petImagePath,
                                  'my_pet_name': widget.petName,
                                  'is_friend_tab': tabController.index == 1,
                                },
                              ).then((_) => setState(() {}));
                            },
                            child: Tooltip(
                              message: AppLocalizations.of(context)!
                                  .pet_agenda_add_event,
  ...
  Widget _buildListTab(List<PetHistoryEntry> entries, AppLocalizations l10n,
      {bool isFriendTab = false}) {
    if (entries.isEmpty) {
      return Center(
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            color: isFriendTab
                ? const Color(0xFFE0BBE4)
                : const Color(
                    0xFFFFD1DC), // Domain Colors: Lilac for Friends, Pink for Pets
          ...
          child: Material(
            ...
            child: InkWell(
              ...
              onTap: () {
                ...
                debugPrint(
                    '[SCAN_NUT_DEBUG] Raw JSON Snippet: ${pet.rawJson.length > 100 ? pet.rawJson.substring(0, 100) : pet.rawJson}...');
                ...
                final breedPatterns = [
                  ...
                  RegExp(PetConstants.regexBreedPt,
                      caseSensitive: false), // Portuguese
                  RegExp(PetConstants.regexBreedEn,
                ...
                try {
                  cleanResult = pet.rawJson
                      .replaceAll(
                          RegExp(r'\[METADATA\](.*?)\[END_METADATA\]',
                              dotAll: true),
                          '')
                      .trim();
                  ...
                  final petNameMatch =
                      RegExp(r'my_pet_name:\s*([^\|\n]*)', caseSensitive: false)
                          .firstMatch(pet.rawJson);
                  safeMyPetName = petNameMatch?.group(1)?.trim() ?? '';

                  final tutorNameMatch =
                      RegExp(r'tutor_name:\s*([^\|\n]*)', caseSensitive: false)
                          .firstMatch(pet.rawJson);
                  ...
                  try {
                    final petBox =
                        ObjectBoxManager.currentStore.box<PetEntity>();
                    debugPrint(
                        '[SCAN_NUT_TRACE] Querying PetEntity for UUID: ${pet.petUuid}');
                    final petEntity = petBox
                        .query(PetEntity_.uuid.equals(pet.petUuid))
                        .build()
                        .findFirst();

                    if (petEntity == null) {
                      debugPrint(
                          '[SCAN_NUT_TRACE] PetEntity NOT FOUND in DB for UUID: ${pet.petUuid}');
                    } else {
                      debugPrint(
                          '[SCAN_NUT_TRACE] PetEntity Found. Tutor Name in DB: "${petEntity.tutorName}"');
                      if (petEntity.tutorName != null &&
                          petEntity.tutorName!.isNotEmpty) {
                        safeTutorName = petEntity.tutorName!;
                      }
                    }
                  } catch (e) {
                    debugPrint(
                        '[SCAN_NUT_WARN] Could not fetch native PetEntity for tutorname fallback: $e');
                  }
                } catch (e) {
                  debugPrint(
                      '[SCAN_NUT_WARN] RegExp Parsing Error in History tap: $e');
                  cleanResult = pet.rawJson; // Fallback to raw if regex breaks
                }

                final ext = pet.imagePath.isNotEmpty
                    ? pet.imagePath.split('.').last.toLowerCase()
                    : '';
                final isVideo = pet.imagePath.isNotEmpty &&
                    PetConstants.videoExtensions.contains(ext);
                final isOcr = pet.category == PetConstants.typeLabel ||
                    pet.category == PetConstants.typeLab ||
                    pet.category == PetConstants.typeLabel.toLowerCase() ||
                    pet.category == PetConstants.typeLab.toLowerCase();
                    ...
                    MaterialPageRoute(
                      builder: (_) => UniversalOcrResultView(
                        ...
                        petDetails: {
                          ...
                          PetConstants.keyIsFriend:
                              isFriendTab ? 'true' : 'false',
                          'my_pet_name': widget.petName != null &&
                                  widget.petName!.isNotEmpty
                              ? widget.petName!
                              : safeMyPetName,
                          PetConstants.keyTutorName: safeTutorName,
                          PetConstants.keyPageTitle:
                              "${pet.category.toCategoryDisplay(context)}: ${pet.petName}",
                        },
                      ),
                    ),
                  );
                            ...
                            MaterialPageRoute(
                              builder: (context) => UniversalPdfPreviewScreen(
                                ...
                                petDetails: {
                                  ...
                                  PetConstants.fieldBreed: breed,
                                  PetConstants.keyPageTitle:
                                      pet.category.toCategoryDisplay(context),
                                  PetConstants.keyTutorName: safeTutorName,
                                  PetConstants.keyIsFriend:
                                      isFriendTab ? 'true' : 'false',
                                },
                        ...
                        children: [
                          Text(
                            isFriendTab
                                ...
                                : (pet.getPlantTitle(context) ??
                                    pet.category
                                        .toCategoryDisplay(context)
                                        .toUpperCase()),
                            style: const TextStyle(
                                color: Colors.black,
                                fontWeight: FontWeight.w900,
    ...
    if (displayImage != null) {
      ...
      return Container(
        ...
        decoration: BoxDecoration(
            ...
            border: Border.all(color: Colors.black, width: 2.5)),
        child: Center(
            child: Icon(isVideo ? Icons.videocam : Icons.broken_image,
                color: Colors.black)),
      );
    }
  }
